...
with a unit and a daily target, so it needs no separate feature and lives here under 习惯打卡, not as a
card on the 学习 page (the card that used to sit there was mislabelled: it opened the habit check-in
screen while the top bar still said 背单词). Memos are two-column cards.
Milestones are a timeline grouped by year. Important dates lead with a countdown card for the
nearest one and support both 阳历 and **农历** (leap months included), a repeat rule, a reminder — which
now fires a real in-app notification on the reminder day, not merely an entry in the exported .ics —
and a per-entry **.ics export** that the phone's calendar can import. A 「只过一次」 date no longer
vanishes the day after it passes: it moves into a **「已经过去」** group that reads 已过去 N 天.
...
The timetable is a 7 × 12 week table whose blocks are coloured by course and sized by their period
range, driven by a 周次表达式 such as `1-16` or `1-16单`; the weekday header and the grid scroll
sideways together, so the day labels stay over the right columns. Assignments are the to-do list filtered to
...
to grade points with the 4.0 / 5.0 口径 you selected, the weekly report summarises the last 7 days and
exports itself as a PNG — with its 「近 7 天」 figure and its bar chart taken from the same window.
...
page says exactly that instead of pretending otherwise. The 关于 page itself no longer carries its
own update row: that row duplicated the 检查更新 entry under 设置 ▸ 关于与更新, so it was removed,
and there is one place to go for updates instead of two.
...
file, and load it back later (for example after changing phones). The to-do's course name, priority
and manual order all round-trip now, so an 作业/DDL entry keeps its course attribution after a restore.
The file is written to a location you pick through the system file picker, so the app still needs
no storage permission. Restoring replaces the current data, and asks for confirmation first — and the
confirmation names every module it really touches rather than claiming only 记账 / 待办 / 专注, because
a restore clears all sixteen tables; 清空全部数据 and 清除所有待办 name their real scope the same way.
...
| **APK size** | About 2 MB (v1.10 grows by very little: the timetable grid, the weekly report, both widgets and the background image are all drawn with framework and Compose APIs) |
...
| **Permissions** | Exactly three: notifications, vibration, and rescheduling reminders after a reboot (to-dos, the nightly ledger reminder and the periodic recap). No storage permission: backups, CSV files, monthly reports, the .ics export and the weekly-report PNG are written through the system file picker. v1.10 adds no permission at all — the important-date reminder rides the same inexact alarm on its own notification channel, the timetable table, the re-armed class reminder and the extra notification are all framework APIs, and 检查更新 merely opens a browser |
...
| **Your data** | Stored only on your device. Uninstalling the app deletes it. Upgrading keeps it — the database migrates automatically (now version 7, which adds `todos.courseName` and eleven tables: memos, milestones, important dates, habits, habit check-ins, courses, exams, revision tasks, grades, credit targets and awards), and rows written by older versions get safe defaults (no tags, not reimbursable, CNY at rate 10000, priority normal, no subtasks, no course name). v1.10 changes no schema at all: it is still version 7, so upgrading from v1.9 runs no migration |
