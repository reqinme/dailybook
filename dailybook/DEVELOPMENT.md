...
（备份与 CSV 都走系统文件选择器）。v1.5 新增的预算预警、定期小结、专注目标提醒，以及通知上的
「稍后提醒」，全部走已有的 `POST_NOTIFICATIONS` 和同一套非精确闹钟，**没有新增任何权限**。
...
- **单一数据源**：专注统计（今日/连续/累计、本周/本月、热力图）全部从 `focus_sessions` 表实时派生，
  不再另存一份计数；主题/界面语言/月度预算/分类预算/专注目标只有一份全局单例设置。
...
- **CSV 导出**：开头写 UTF-8 BOM、行尾用 CRLF、含逗号/引号/换行的单元格加引号转义，
  这样 Excel 双击打开中文不乱码，排序按日期 + id 固定。表头固定是「日期 / 类型 / 分类 / 账户 / 金额 / 备注」
  这六列，但**文字随界面语言**：中文界面写 `日期,类型,分类,账户,金额,备注`，英文界面写
  `Date,Type,Category,Account,Amount,Note`，`类型` 列的内容（支出 / 收入）也是该语言的译文；
  列数与列序在四种语言下完全一致（`I18nTest` 会替每种语言验一遍表头与数据行都是 6 列）。
  想在 Excel 里按账户对账直接筛账户列即可。
