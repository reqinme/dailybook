...
- 未做真机运行验证（构建机没有连接安卓设备），**v1.7 的代码全部落地后要重新跑一遍完整构建再定稿**：
  `gradle testDebugUnitTest`（当前仓库里 62 个用例）、`gradle lintRelease` 与 `gradle assembleRelease`，
  并核对数据库迁移 SQL（含 v4→v5 的七条加列、v5→v6 的两条加列与两张新表）的结构。
  上面「功能一览 / 技术栈 / 单元测试」里的数字都按写完这份文档时的仓库状态填的，
  改完代码要回来对齐（用例数、APK 体积、dist 里的文件名）。
